...
public class USLoadingEffect : MonoBehaviour
{
    ...
    void UGUILoading()
    {
        if (Loading)
        {
			...
			// Edit for VR
			//this.transform.Rotate(0f, 0f, 8f, Space.World);
			//this.transform.Rotate(0f, 0f, Time.deltaTime * 400f, Space.World);
			this.transform.localEulerAngles = new Vector3 (this.transform.localEulerAngles.x, this.transform.localEulerAngles.y, this.transform.localEulerAngles.z + 8f);
